## use this script to generate obj/ bg seg features for patched ViT
import sys
...
def generate_features(df_path, img_dir, do_obj_seg, save_path, model_name="vit_base_patch16_224", do_full_img=False, use_blackened = False, use_compliment_for_BG = True):
        ...
        if img_path.is_file():
            try: 
                ...
                with torch.no_grad():
                    ...
                    image_pil = preprocessor2(image_pil)
                    image_pil = image_pil.cuda()

                    patch_mask = patch_mask[0,:].unsqueeze(0).cuda()
                    # extracting features of original image, patch mask from obj_seg image
                    feature = transformer.forward_features(image_pil.unsqueeze(0), patch_mask)[0].cpu().detach().numpy()
                    features.append(
                        feature
                    )
                    is_good_features.append(1)
...
#     generate_features_new(args.dataset_common_objects_df_path, args.img_dir, args.do_obj_seg, args.save_path, args.model_name, args.use_blackened, args.use_compliment_for_BG)
